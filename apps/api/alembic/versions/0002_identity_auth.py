...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_organizations_slug", "organizations", ["slug"], unique=True, schema="identity"
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["active_organization_id"], ["identity.organizations.id"], ondelete="SET NULL"
        ),
        schema="identity",
    )
    ...
    op.create_index(
        "ix_memberships_organization_id", "memberships", ["organization_id"], schema="identity"
    )
    ...
    op.create_index(
        "ix_projects_organization_id", "projects", ["organization_id"], schema="identity"
    )
    ...
    op.create_index(
        "ix_refresh_token_hash", "refresh_tokens", ["token_hash"], unique=True, schema="identity"
    )
    op.create_index(
        "ix_refresh_tokens_family_id", "refresh_tokens", ["family_id"], schema="identity"
    )
    ...
    op.create_index(
        "ix_api_keys_organization_id", "api_keys", ["organization_id"], schema="identity"
    )
    ...
    op.create_index(
        "ix_audit_logs_organization_id", "audit_logs", ["organization_id"], schema="identity"
    )
